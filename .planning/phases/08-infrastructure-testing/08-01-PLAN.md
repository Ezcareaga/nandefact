...
<task type="auto">
  ...
  <action>
    ...
    7. Create initial migration: Run `npx prisma migrate dev --name init` to generate and apply the initial migration. This creates the `prisma/migrations/` directory with the SQL migration file and applies it to the local PostgreSQL (requires `docker compose up -d postgres` running). If PostgreSQL is not running yet, at minimum run `npx prisma validate` and `npx prisma generate` — the migration will be created in Task 2 after docker-compose is updated.
  </action>
  <verify>
    - `npx prisma validate` passes without errors
    - `npx prisma generate` generates client successfully
    - `npx prisma migrate dev --name init` creates migration and applies to PostgreSQL (if running)
