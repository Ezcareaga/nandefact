...
<task type="auto">
  ...
  <action>
    1. Add `"test:e2e": "vitest run tests/e2e/"` to package.json scripts.
       Also add `"test:all": "vitest run"` that runs everything (unit + integration + e2e).

    2. **Create `tests/e2e/helpers/testServer.ts`:**
    ...
    3. **Create `tests/e2e/facturaFlow.test.ts`:**
       The test follows the core user journey described in CLAUDE.md:

       **Setup (beforeAll):**
       - Start test server via `createTestServer()`
       - Seed database with:
         a) A comercio (Almacén Doña María, RUC 80069563-1, timbrado vigente, establecimiento 001, punto 003)
         b) A usuario (dueño, teléfono +595981234567, PIN hash for "1234")
         c) 3 productos (Mandioca Gs 5000/kg IVA 5%, Arroz Gs 8000/kg IVA 10%, Aceite Gs 15000/un IVA 10%)
         d) A cliente (Juan Pérez, CI 4567890)
    ...
    4. **HTTP request helper:**
       Use native `fetch` (Node 20+ has it built-in) or import `supertest` (add as dev dependency if not present).
       Recommendation: Use Node's built-in `fetch` to avoid adding dependencies. The test server listens on a random port.
       Alternative: Install supertest (`npm install -D supertest @types/supertest`) for cleaner test syntax.
       Choose supertest — it's standard for Express testing and already listed in CLAUDE.md stack.
  </action>
  <verify>
