...
files_modified:
  ...
  - nandefact-api/src/domain/factura/IClienteRepository.ts
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "nandefact-api/src/application/sync/SincronizarPendientes.ts"
      provides: "Updated use case with IXmlGenerator dependency"
      contains: "xmlGenerator"
    - path: "nandefact-api/src/domain/factura/IClienteRepository.ts"
  ...
  key_links:
    ...
    - from: "nandefact-api/src/application/facturacion/EnviarDE.ts"
      to: "nandefact-api/src/domain/factura/IClienteRepository.ts"
      via: "dependency injection"
...
<task type="auto">
  ...
  <action>
**Step 1: Create IClienteRepository port** in `src/domain/factura/IClienteRepository.ts`:

NOTE: Place this alongside other factura-related ports for now. Alternative location would be `src/domain/cliente/IClienteRepository.ts` but following existing pattern where IFacturaRepository is in the factura directory.

Actually, check existing IComercioRepository location — it's in `src/domain/comercio/IComercioRepository.ts`. Follow the same pattern and create `src/domain/cliente/IClienteRepository.ts`:
...
Wait — check the planning context. The IClienteRepository path in CLAUDE.md says `src/domain/cliente/IClienteRepository.ts`. Use that location.
